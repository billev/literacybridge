...
// Transfer method:	stack
//========================================================
unsigned int Snd_A1800_GetMode( int handle )
{
...
// Transfer method:	stack
//========================================================
unsigned int Snd_A4800_GetMode( int handle )
{
...
unsigned long Snd_A1800_GetTotalTime(unsigned long DataPtr)
{
	...
	SACM_curSpeed = ( unsigned int )Snd_GetSpeed();
//	SACM_curSpeed -= 12;					//The Middle Speed is 12
	//The speed dispose
	if(SACM_curSpeed < 0)
	{
		uiTempSpeed = -SACM_curSpeed;
		ulTotalTime = (unsigned long)( (float)(ulTotalTime * 20) / powf((float)2, (float)uiTempSpeed/(float)12) );
	}
	else
	{
		ulTotalTime = (unsigned long)( (float)(ulTotalTime * 20) / powf((float)2, (float)SACM_curSpeed/(float)12) );
	}
...
//	SACM_curSpeed -= 12;					//The Middle Speed is 12
	...
	{
		uiTempSpeed = -SACM_curSpeed;
		ulTotalTime = (unsigned long)( (float)(ulTotalTime << 4) / powf((float)2, (float)uiTempSpeed/(float)12) );	//16ms
	}
	else
	{
		ulTotalTime = (unsigned long)( (float)(ulTotalTime << 4) / powf((float)2, (float)SACM_curSpeed/(float)12) );	//16ms
	}
	return ulTotalTime;
}
...
unsigned long Snd_A1800_GetCurrentTime(void)
{
	...
	unsigned int FrameNum, uiWordsPerFrame, SACM_curSpeed;
	...
	SACM_curSpeed = ( unsigned int )Snd_GetSpeed();
//	SACM_curSpeed -= 12;					//The Middle Speed is 12
	ulReturnTime = (unsigned long)( ulFrameNumber / powf((float)2, (float)SACM_curSpeed/(float)12) );
	return ulReturnTime;
}
...
//--------------------------------------------------------------------
// Syntax:		unsigned long Snd_Get_A1800SkipOffset(unsigned long StartTime)
...
// Syntax:		int Snd_SACM_PlayFAT( int _CodecType,int FileHandle)
...
// Transfer method:	stack
//========================================================
int Snd_SACM_PlayFAT( int _CodecType,int FileHandle)
{
...
// Syntax:		Snd_SACM_RecMemory(unsigned long RECStartAddr, int _CodecType, int BitRate)
...
{	
	SACM_Codec(_CodecType);
	if(C_CODEC_WAV == _CodecType)
	{
		USER_SetRECStartAddr(RECStartAddr);
	}
	else
	{
...
//========================================================
// Function Name:	Snd_Pause
// Syntax:		Snd_Pause(unsigned int PlayID)
// Purpose: 	stop the playing algorithms			 
// Note:		NO
// Parameter : 	None
// Return:		N/A
...
// Transfer method:	stack
//========================================================
unsigned int Snd_Pause(void)
{
	SACM_Pause();
	return TRUE;
}
//========================================================
// Function Name:	Snd_Resume
// Syntax:		Snd_Resume(unsigned int PlayID)
// Purpose: 	stop the playing algorithms			 
// Note:		NO
// Parameter : 	None
// Return:		N/A
...
// Syntax:		Snd_GetStatus(unsigned int PlayID)
// Purpose: 	get the status of sound module			 
// Note:		NO
// Parameter : 	None
// Return:		 The Status of Sound Module of C_Snd_Stop/C_Snd_Pause/C_Snd_DVRing/C_Snd_Playing
// Destory: 	N/A
// Stack Depth:  
// CPU Cycle:
// Transfer method:	Stack
//========================================================
unsigned int Snd_GetStatus(void)
{
	unsigned int uiStatus;

	uiStatus = SACM_Status();
	if(uiStatus & C_SACM_STOP)
	{
		return(C_Snd_Stop);
	}
	else if( (uiStatus & C_SACM_PLAY) && (uiStatus & C_SACM_PAUSE) )
	{
		return(C_Snd_Pause);
	}
	else if( (uiStatus & C_SACM_PLAY) && (!(uiStatus & C_SACM_PAUSE)) )
	{
		if( uiStatus & C_SACM_CODEC_MODE )
		{
			return(C_Snd_DVRing);
		}
		else
		{
			return(C_Snd_Playing);
		}
	}
...
{		
	...
	write(SACMFileHandle, (unsigned long)WriterBuffer<< 1, HeaderLength<< 1);
}	
#endif
